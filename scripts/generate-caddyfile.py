#!/usr/bin/env python3
"""Generate a Caddyfile from routes.json.

routes.json is a list of projects, each with a list of path routes:

    [
        {
            "projectName": "Flowlet",
            "launchScript": "$HOME/Flowlet/scripts/launch.sh",
            "paths": [
                {"path": "/api/*", "port": 8000},
                {"path": "/*",     "port": 3000}
            ]
        }
    ]

Each path entry forwards requests matching `path` to 127.0.0.1:<port>.
A path ending in "/*" also covers the bare prefix: "/flowlet" is
308-redirected to "/flowlet/", so a trailing slash is never required
(and backends that only serve the slashed path still work). The
redirect is skipped if an explicit "/flowlet" route exists.

"launchScript" and "appPath" are not used here — they are read by
scripts/panel.py, the control panel served at /default that can
start/stop projects (and git pull --rebase in "appPath" on update). The
/default prefix is reserved: a route for it (to the panel's port,
$PANEL_PORT or 8090) is injected automatically.

Optional per-path keys:
    "strip": true   — strip the matched prefix before forwarding
                      (Caddy `handle_path`). Default false: the backend
                      receives the full original path (Caddy `handle`),
                      which is what apps mounted under a prefix expect.

Routes are emitted longest-path-first so more specific paths win
(/flowlet/api/* is matched before /flowlet/*). A 404 fallback is added
unless a route already covers everything (path "/*" or "/").

The panel path defaults to /default but is configurable (--panel-path or
$PANEL_PATH) so two instances can coexist on one host; --admin-port
($ADMIN_PORT) names a distinct Caddy admin endpoint for the same reason.

Usage:
    generate-caddyfile.py [routes.json] [-o Caddyfile] [--listen 8080]
                          [--panel-port 8090] [--panel-path /default]
                          [--admin-port 2019]
"""

import argparse
import json
import os
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent
DEFAULT_PANEL_PATH = "/default"
DEFAULT_ADMIN_PORT = "2019"  # Caddy's own default admin endpoint port


def normalize_panel_path(path):
    """Normalize a panel path: ensure a single leading '/', no trailing '/'."""
    path = "/" + path.strip().strip("/")
    if path == "/":
        fail("panel path must be a non-empty path like '/default'")
    return path


def fail(msg):
    print(f"error: {msg}", file=sys.stderr)
    sys.exit(1)


def load_routes(config_path, panel_path):
    try:
        projects = json.loads(Path(config_path).read_text())
    except FileNotFoundError:
        fail(f"config not found: {config_path}")
    except json.JSONDecodeError as e:
        fail(f"invalid JSON in {config_path}: {e}")

    if not isinstance(projects, list) or not projects:
        fail("config must be a non-empty JSON array of projects")

    routes = []
    seen_paths, seen_names = set(), set()
    for i, project in enumerate(projects):
        where = f"project #{i + 1}"
        if not isinstance(project, dict):
            fail(f"{where}: must be an object")

        unknown = set(project) - {"projectName", "launchScript", "appPath",
                                  "envFile", "paths"}
        if unknown:
            fail(f"{where}: unknown keys: {', '.join(sorted(unknown))}")

        name = project.get("projectName")
        if not isinstance(name, str) or not name.strip():
            fail(f"{where}: \"projectName\" must be a non-empty string")
        if name in seen_names:
            fail(f"{where}: duplicate projectName {name!r}")
        seen_names.add(name)
        where = f"project {name!r}"

        launch = project.get("launchScript")
        if launch is not None and (not isinstance(launch, str) or not launch.strip()):
            fail(f"{where}: \"launchScript\" must be a non-empty string")

        env_file = project.get("envFile")
        if env_file is not None and (not isinstance(env_file, str)
                                     or not env_file.strip()):
            fail(f"{where}: \"envFile\" must be a non-empty string")

        paths = project.get("paths")
        if not isinstance(paths, list) or not paths:
            fail(f"{where}: \"paths\" must be a non-empty array")

        for j, route in enumerate(paths):
            rwhere = f"{where}, path #{j + 1}"
            if not isinstance(route, dict):
                fail(f"{rwhere}: must be an object")

            unknown = set(route) - {"path", "port", "strip"}
            if unknown:
                fail(f"{rwhere}: unknown keys: {', '.join(sorted(unknown))}")

            path = route.get("path")
            if not isinstance(path, str) or not path.startswith("/"):
                fail(f"{rwhere}: \"path\" must be a string starting with '/'")
            if path in seen_paths:
                fail(f"{rwhere}: duplicate path {path!r}")
            seen_paths.add(path)
            if path.rstrip("*").rstrip("/") == panel_path or path.startswith(panel_path + "/"):
                fail(f"{rwhere}: {panel_path} is reserved for the control panel")

            port = route.get("port")
            if not isinstance(port, int) or not 1 <= port <= 65535:
                fail(f"{rwhere} ({path}): \"port\" must be an integer in 1-65535")

            if not isinstance(route.get("strip", False), bool):
                fail(f"{rwhere} ({path}): \"strip\" must be true or false")

            routes.append(route)

    return routes


def render(routes, listen_port, panel_port, panel_path, admin_port):
    routes = routes + [{"path": panel_path + "/*", "port": panel_port}]
    routes = sorted(routes, key=lambda r: len(r["path"]), reverse=True)
    has_catch_all = any(r["path"] in ("/", "/*") for r in routes)

    global_block = ["\tauto_https off"]
    # A second Caddy on the same host would clash on the default admin
    # endpoint (localhost:2019), so name a distinct one per instance. Omit
    # it at the default so single-instance Caddyfiles stay unchanged.
    if admin_port != DEFAULT_ADMIN_PORT:
        global_block.append(f"\tadmin localhost:{admin_port}")

    lines = [
        "# Generated by scripts/generate-caddyfile.py from routes.json — do not edit by hand.",
        "# Regenerate + deploy with: scripts/apply.sh",
        "{",
        *global_block,
        "}",
        "",
        f":{listen_port} {{",
    ]
    all_paths = {r["path"] for r in routes}
    for route in routes:
        path, port = route["path"], route["port"]
        directive = "handle_path" if route.get("strip") else "handle"
        # Redirect the bare prefix ("/flowlet" -> "/flowlet/") so a trailing
        # slash is never required, even for backends that only serve the
        # slashed path. Caddy orders redir before handle, so an explicit
        # route for the bare path must suppress the redirect.
        base = path[:-2] if path.endswith("/*") and len(path) > 2 else None
        if base and base not in all_paths and base + "/" not in all_paths:
            lines.append(f"\tredir {base} {base}/ 308")
        lines += [
            f"\t{directive} {path} {{",
            f"\t\treverse_proxy 127.0.0.1:{port}",
            "\t}",
        ]
    if not has_catch_all:
        lines += [
            "\thandle {",
            '\t\trespond "no route configured for this path" 404',
            "\t}",
        ]
    lines.append("}")
    return "\n".join(lines) + "\n"


def main():
    parser = argparse.ArgumentParser(description=__doc__.splitlines()[0])
    parser.add_argument("config", nargs="?", default=str(ROOT / "routes.json"),
                        help="path to routes.json (default: repo root)")
    parser.add_argument("-o", "--output", default="-",
                        help="output file (default: stdout)")
    parser.add_argument("--listen", type=int,
                        default=int(os.environ.get("PROXY_PORT", "8080")),
                        help="port Caddy listens on (default: $PROXY_PORT or 8080)")
    parser.add_argument("--panel-port", type=int,
                        default=int(os.environ.get("PANEL_PORT", "8090")),
                        help="port the control panel listens on "
                             "(default: $PANEL_PORT or 8090)")
    parser.add_argument("--panel-path",
                        default=os.environ.get("PANEL_PATH", DEFAULT_PANEL_PATH),
                        help="URL prefix the control panel is served at "
                             "(default: $PANEL_PATH or /default)")
    parser.add_argument("--admin-port",
                        default=os.environ.get("ADMIN_PORT", DEFAULT_ADMIN_PORT),
                        help="Caddy admin API port; emitted as 'admin "
                             "localhost:<port>' so multiple instances don't "
                             "clash (default: $ADMIN_PORT or 2019)")
    args = parser.parse_args()

    panel_path = normalize_panel_path(args.panel_path)
    caddyfile = render(load_routes(args.config, panel_path), args.listen,
                       args.panel_port, panel_path, args.admin_port)
    if args.output == "-":
        sys.stdout.write(caddyfile)
    else:
        Path(args.output).write_text(caddyfile)
        print(f"wrote {args.output}", file=sys.stderr)


if __name__ == "__main__":
    main()
